...
void DataInitInstance::beginDataInit(){
    ...
    while (getline(myfile, str)) {
        ...
        if (pos == 0) {
            ...
            }else if("DBHost"== vec[0]){
                db_host=vec[1];
        }else if("DBUser"==vec[0]){
               db_user=vec[1];
        }else if("DBPWD"==vec[0]){
               db_pwd=vec[1];
        }else if("DBMaxConnSize"==vec[0]){
               db_maxConnSize=boost::lexical_cast<int>(vec[1]);
        }else if("DBNAME"==vec[0]){
                db_name = vec[1];
        }else if("DBPORT"==vec[0]){
                db_port=boost::lexical_cast<int>(vec[1]);
        }else if("DBCharSet"==vec[0]){
                db_charset=vec[1];
    ...
    g_db_pool = std::make_shared<MysqlConnectPool>(db_name, db_host, db_user, db_pwd, db_port, db_charset, db_maxConnSize);
...
void DataInitInstance::saveThostFtdcOrderFieldToDb(CThostFtdcOrderField *pOrder){
    try{
       ...
       //instrumentid,instrumentid_ch,variety_ch,price,volume,direction,offsetflag,brokerid,exchangeid,investorid,
       //tradetype,tradedatetime,tradeid,tradedatetime_ctp

       row.instrumentid = pOrder->InstrumentID;
      vector<string> res = getInstrumentIDZH(row.instrumentid);
       row.instrumentid_ch=res[1];
       row.variety_ch=res[0];
